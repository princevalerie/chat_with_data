...
def create_thinking_agent(api_key):
    ...
    class ThinkingAgent:
        def __init__(self, api_key):
            ...
            self.llm = Gemini(api_key=api_key, model="gemini-2.0-flash")
        ...
        def analyze_query(self, query, data_info):
            ...
            try:
                ...
                if "compare" in query.lower() or "bandingkan" in query.lower() or "perbandingan" in query.lower():
                    vizualization_needs.append({"description": "Visualisasi untuk membandingkan nilai antar kategori"})
                    
                if "distribution" in query.lower() or "distribusi" in query.lower():
                    vizualization_needs.append({"description": "Visualisasi untuk menunjukkan distribusi data"})
                    
                if "relationship" in query.lower() or "hubungan" in query.lower() or "korelasi" in query.lower():
                    vizualization_needs.append({"description": "Visualisasi untuk menunjukkan hubungan antar variabel"})
                    
                if "part of whole" in query.lower() or "bagian dari" in query.lower() or "proporsi" in query.lower():
                    vizualization_needs.append({"description": "Visualisasi untuk menunjukkan proporsi atau bagian dari keseluruhan"})
                
                # Untuk query umum, tambahkan visualisasi umum
                if not vizualization_needs:
                    vizualization_needs.append({"description": "Visualisasi yang paling informatif sesuai data"})
                ...
                if any(word in query.lower() for word in ["detail", "mendalam", "komprehensif", "lengkap", "advanced", "lanjut", "complex"]):
                    if len(vizualization_needs) < 2:
                        vizualization_needs.append({"description": "Visualisasi tambahan untuk menunjukkan aspek lain dari data"})
                        vizualization_needs.append({"description": "Visualisasi untuk insight tersembunyi dalam data"})
                ...
                analysis["visualizations"] = vizualization_needs
